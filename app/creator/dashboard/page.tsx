...
export default async function CreatorDashboardPage() {
  ...
  const [{ data: profileData, error: profileError }, { data: usageData, error: usageError }] = await Promise.all([
    supabase!.from('creator_profiles').select('id,full_name,niche,location,average_views,engagement_rate').eq('user_id', account.id).maybeSingle(),
    supabase!.from('usage_limits').select('free_evaluations_total,free_evaluations_used').eq('user_id', account.id).maybeSingle(),
  ]);
  if (profileError || usageError) throw new Error('Creator dashboard data is temporarily unavailable.');
  ...
  if (profile) {
    ...
    if (expiryError) throw new Error('Creator dashboard activity is temporarily unavailable.');
  }
  ...
  const evaluationsRemaining = Math.max(0, (usage?.free_evaluations_total ?? 5) - (usage?.free_evaluations_used ?? 0));
  ...
  return (
    <AppShell role="creator" displayName={name} email={account.email} plan={account.plan}>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[0.1em] text-violet-700">Creator workspace</p><h1 className="mt-1 text-2xl font-bold tracking-[-0.035em] text-slate-950 sm:text-3xl">Welcome back, {firstName(name)}</h1><p className="mt-2 text-sm text-slate-500">Here is what needs your attention across opportunities and offers.</p></div>
        <Link className="mt-3 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-blue-600 px-4 text-sm font-semibold text-white shadow-sm sm:mt-0" href="/creator/ai-advisor"><Bot className="size-4" aria-hidden="true" />Analyze a deal</Link>
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,0.75fr)]">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-violet-700">Top offer for you</p><h2 className="mt-1 text-lg font-bold text-slate-950">Highest-priority active offer</h2></div>{topOffer ? <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-700">{topOffer.status.replaceAll('_', ' ')}</span> : null}</div>
          {topOffer ? <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:p-5"><div className="grid gap-4 sm:grid-cols-3"><DealValue label="Cash payment" value={formatInr(topOffer.cash_payment)} /><DealValue label="Product perk" value={topOffer.product_name ?? 'None'} /><DealValue label="Product value" value={formatInr(topOffer.product_value)} /></div><div className="mt-5 flex flex-wrap gap-2"><Link className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-semibold text-white" href={`/creator/offers/${topOffer.id}`}>View offer<ArrowRight className="size-4" /></Link><Link className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-800" href={`/creator/ai-advisor?offer=${topOffer.id}`}><Sparkles className="size-4 text-violet-700" />Analyze with AI</Link></div></div> : <div className="mt-5"><EmptyState icon={Handshake} title="No offers yet" description="Your structured offers will appear here when a brand sends one." /></div>}
        </section>

        <aside className="rounded-2xl border border-slate-200 bg-slate-950 p-5 text-white shadow-sm sm:p-6">
          <span className="flex size-10 items-center justify-center rounded-xl bg-white/10 text-violet-300"><Sparkles className="size-5" aria-hidden="true" /></span><p className="mt-6 text-xs font-bold uppercase tracking-[0.1em] text-violet-300">AI evaluations remaining</p><strong className="mt-2 block text-5xl font-bold tracking-[-0.05em]">{evaluationsRemaining}</strong><p className="mt-2 text-sm leading-6 text-slate-300">Every account starts with 5 free evaluations. Browsing stays available when they run out.</p><Link className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-white" href="/creator/subscription">View current plan<ArrowRight className="size-4" /></Link>
        </aside>
      </div>

      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-blue-700">Recent opportunities</p><h2 className="mt-1 text-lg font-bold text-slate-950">Campaigns matched to your profile</h2></div><Link className="text-sm font-semibold text-violet-700" href="/creator/opportunities">View all</Link></div><div className="mt-5"><EmptyState icon={Megaphone} title="We haven’t found a strong match yet" description="New published campaigns that fit your niche, audience, size, engagement, budget, and platform will appear here." actionLabel="Browse opportunities" actionHref="/creator/opportunities" /></div></section>
    </AppShell>
  );
}
