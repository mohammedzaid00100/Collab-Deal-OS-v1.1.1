  ...
  return <AppShell role="creator" displayName={account.displayName ?? 'Creator'} email={account.email} plan={account.plan}>
    ...
    {deals.length ? <section className="mt-7 grid gap-4 lg:grid-cols-2" aria-label="Published deals">{deals.map((deal) => {
      ...
      return <article className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-md" key={deal.id}>
        <div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-emerald-700"><Radio className="size-3" />Live deal</span><span className="text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">{deal.platform} · {deal.deal_type.replaceAll('_', ' ')}</span></div><h2 className="mt-3 text-lg font-bold tracking-[-0.025em] text-slate-950">{deal.title}</h2><p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-500">{deal.description}</p></div><Link className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-slate-50 text-slate-500 transition group-hover:bg-slate-950 group-hover:text-white" href={`/creator/connect/${deal.id}`} aria-label={`Open ${deal.title}`}><ArrowRight className="size-4" /></Link></div>
