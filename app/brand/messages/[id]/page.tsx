...
export default async function BrandConversationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell role="brand" displayName={account.displayName ?? 'Brand'} email={account.email} plan={account.plan}>
    ...
    <section className="mt-3 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <header className="flex items-center gap-3 border-b border-slate-200 px-4 py-4 sm:px-5"><span className="flex size-10 items-center justify-center rounded-full bg-gradient-to-br from-violet-100 to-blue-100 font-bold text-violet-700">{creator.full_name.slice(0, 1).toUpperCase()}</span><div className="min-w-0 flex-1"><h1 className="truncate text-sm font-bold text-slate-950">{creator.full_name}</h1><p className="truncate text-xs text-slate-500">@{creator.username} · {creator.niche} · {creator.location}</p></div><Link className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700" href={`/brand/connect/${conversation.campaign_id}`}>View deal</Link></header>
      ...
      <div className="min-h-[420px] max-h-[62vh] overflow-y-auto p-4 sm:p-5">{messages.length ? <div className="grid gap-3">{messages.map((message) => { const own = message.sender_user_id === account.id; return <div className={`flex ${own ? 'justify-end' : 'justify-start'}`} key={message.id}><div className={`max-w-[80%] rounded-2xl px-4 py-3 ${own ? 'rounded-br-md bg-slate-950 text-white' : 'rounded-bl-md bg-slate-100 text-slate-800'}`}><p className="whitespace-pre-wrap text-sm leading-6">{message.body}</p><span className={`mt-1 block text-[10px] ${own ? 'text-slate-400' : 'text-slate-400'}`}>{formatTime(message.created_at)}</span></div></div>; })}</div> : <div className="flex min-h-[360px] flex-col items-center justify-center text-center"><MessageCircle className="size-8 text-slate-300" /><h2 className="mt-3 text-sm font-bold text-slate-800">Start the conversation</h2><p className="mt-1 max-w-sm text-xs leading-5 text-slate-500">Discuss the deal details, expectations, timeline, and next steps here.</p></div>}</div>
