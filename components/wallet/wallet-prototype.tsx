...
export function WalletPrototype({ role }: { role: AccountType }) {
  ...
  const helper = useMemo(() => role === 'brand'
    ? 'Add funds before paying creators. You can also request withdrawal of unused prototype funds. Creator payments are made from an eligible brand conversation.'
    : 'Creator earnings appear here after a verified brand payment. Withdrawal requests stay pending until Collab Deal OS completes the payout.', [role]);
  ...
  function settleWithdrawal(transactionId: string) {
    ...
    setWallet(next);
    setNotice('Demo operator settlement completed. The withdrawal is now successful and the wallet balance has been deducted.');
  }
...
function WithdrawalFlow({ step, amount, setAmount, numericAmount, balance, canWithdraw, upiId, setUpiId, upiValid, password, setPassword, error, onNextAmount, onNextUpi, onApprove, onReject, onSubmitPassword, onDone }: {
  ...
  if (step === 'amount') return <>
    ...
    <p className="mt-2 text-xs text-slate-500">Minimum withdrawal: ₹{MIN_WITHDRAWAL}. Available: ₹{balance.toLocaleString('en-IN')}.</p>
  ...
  return <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-6 text-center"><Clock3 className="mx-auto size-10 text-amber-700" /><h3 className="mt-3 text-lg font-bold text-amber-950">Withdrawal pending</h3><p className="mt-2 text-sm leading-6 text-amber-800">Your request has been recorded as pending. The wallet balance is unchanged until Collab Deal OS completes the payout. This is a prototype state only.</p><button className="mt-5 min-h-11 rounded-xl bg-slate-950 px-5 text-sm font-bold text-white" style={{ color: '#fff' }} type="button" onClick={onDone}><CheckCircle2 className="mr-2 inline size-4" />Done</button></div>;
}
