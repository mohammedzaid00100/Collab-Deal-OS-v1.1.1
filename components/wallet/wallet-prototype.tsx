...
import {
  checkPrototypePaymentPassword,
  type PaymentPasswordVerification,
} from '@/lib/prototype-payment-security';
...
export function WalletPrototype({ role }: { role: AccountType }) {
  ...
  const helper = useMemo(() => role === 'brand'
    ? 'Add demo funds before paying creators. Protected wallet actions use your single Collab Deal OS payment password. Razorpay will replace the demo payment layer in the MVP.'
    : 'Add demo funds or review creator earnings in this prototype. Protected wallet actions use your single Collab Deal OS payment password. Razorpay will be connected in the MVP.', [role]);
  ...
  async function verifyPaymentPassword() {
    if (!password) {
      setError('Enter your Collab Deal OS payment password.');
      return false;
    }
    const result = await checkPrototypePaymentPassword(role, password);
    if (!result.valid) {
      setError(paymentPasswordMessage(result));
      return false;
    }
    return true;
  }
  ...
  async function addFunds() {
    ...
    if (!(await verifyPaymentPassword())) return;
  ...
  return <div>
    ...
    <section className="mt-7 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><span className="flex size-10 items-center justify-center rounded-xl bg-violet-50 text-violet-700"><ShieldCheck className="size-5" /></span><h2 className="mt-4 font-bold text-slate-950">Payment security</h2><p className="mt-2 text-sm leading-6 text-slate-500">Add funds and withdrawals use the same payment password as account deletion. Forgot-password recovery verifies the signed-in account by email code.</p></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><span className="flex size-10 items-center justify-center rounded-xl bg-blue-50 text-blue-700"><CreditCard className="size-5" /></span><h2 className="mt-4 font-bold text-slate-950">Prototype UPI verification</h2><p className="mt-2 text-sm leading-6 text-slate-500">The demo rejects malformed and obvious placeholder/fake UPI IDs. Razorpay will perform real verification in the MVP.</p></div>
      </div>
    </section>
    ...
    {dialog ? <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={dialog === 'add' ? 'Add prototype funds' : 'Prototype withdrawal'}>
      <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl sm:p-7">
        ...
        {dialog === 'add' ? <>
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500">Prototype top-up only. No real payment occurs. Razorpay checkout will be connected in the MVP.</p>

          <label className="mt-5 block text-sm font-semibold text-slate-700" htmlFor="wallet-add-password">Payment password</label>
          <div className="mt-2 flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4"><LockKeyhole className="size-4 text-slate-400" /><input id="wallet-add-password" className="min-h-12 w-full bg-transparent px-3 text-slate-950 outline-none" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" placeholder="Payment password" /></div>
          <div className="mt-2 flex justify-end"><button className="text-xs font-bold text-violet-700 hover:underline" type="button" onClick={() => { setError(''); setRecoveryOpen(true); }}>Forgot payment password?</button></div>
          {error ? <p className="mt-3 rounded-xl bg-red-50 px-4 py-3 text-sm font-medium text-red-700" role="alert">{error}</p> : null}
          <button className="mt-6 min-h-12 w-full rounded-xl bg-slate-950 px-4 text-sm font-bold text-white disabled:opacity-40" style={{ color: '#fff' }} type="button" disabled={numericAmount <= 0 || !password} onClick={() => void addFunds()}>Verify password and add demo funds</button>
        </> : <WithdrawalFlow step={withdrawStep} amount={amount} setAmount={setAmount} numericAmount={numericAmount} balance={availableForNewWithdrawal} canWithdraw={canWithdraw} upiId={upiId} setUpiId={changeUpiId} upiFormatValid={upiFormatValid} upiVerified={upiVerified} verification={upiVerification} onVerifyUpi={verifyUpiId} password={password} setPassword={setPassword} error={error} onNextAmount={() => setWithdrawStep('upi')} onNextUpi={() => setWithdrawStep('confirm')} onApprove={() => setWithdrawStep('password')} onReject={resetFlow} onSubmitPassword={() => void approveWithdrawalWithPassword()} onDone={resetFlow} onForgotPassword={() => { setError(''); setRecoveryOpen(true); }} />}
      </div>
    </div> : null}
    ...
    <PaymentPasswordRecoveryDialog
      ...
      onReset={() => { setPassword(''); setError(''); setNotice('Payment password changed. Use the new password to continue.'); }}
    />
  </div>;
}

function WithdrawalFlow({ step, amount, setAmount, numericAmount, balance, canWithdraw, upiId, setUpiId, upiFormatValid, upiVerified, verification, onVerifyUpi, password, setPassword, error, onNextAmount, onNextUpi, onApprove, onReject, onSubmitPassword, onDone, onForgotPassword }: {
  step: WithdrawStep;
  amount: string;
  ...
  if (step === 'upi') return <>
    ...
    <button className="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-4 text-sm font-bold text-violet-800 disabled:cursor-not-allowed disabled:opacity-40" type="button" disabled={!upiFormatValid || verification.status === 'checking'} onClick={onVerifyUpi}>{verification.status === 'checking' ? <><LoaderCircle className="size-4 animate-spin" />Checking UPI ID…</> : <><ShieldCheck className="size-4" />Verify UPI ID</>}</button>
  ...
  if (step === 'confirm') return <>
    ...
    <p className="mt-4 text-sm leading-6 text-slate-600">Check the amount and UPI ID carefully. The next step asks for your single Collab Deal OS payment password.</p>
  ...
  if (step === 'password') return <>
    ...
    <div className="mt-2 flex justify-end"><button className="text-xs font-bold text-violet-700 hover:underline" type="button" onClick={onForgotPassword}>Forgot payment password?</button></div>
    {error ? <p className="mt-3 rounded-xl bg-red-50 px-4 py-3 text-sm font-medium text-red-700" role="alert">{error}</p> : null}
...
function paymentPasswordMessage(result: PaymentPasswordVerification) {
  if (result.reason === 'LOCKED') return 'Payment password verification is temporarily locked after repeated failed attempts. Try again shortly or use Forgot payment password.';
  if (result.reason === 'NOT_CONFIGURED') return 'Create your Collab Deal OS payment password before continuing.';
  if (result.reason === 'INCORRECT') {
    const remaining = typeof result.remainingAttempts === 'number' ? ` ${result.remainingAttempts} attempt${result.remainingAttempts === 1 ? '' : 's'} remaining.` : '';
    return `Incorrect payment password.${remaining}`;
  }
  if (result.reason === 'AUTH_REQUIRED') return 'Sign in again before using payment actions.';
  return 'Payment password verification is temporarily unavailable.';
}
...
function prototypeValidateUpi(value: string): { valid: boolean; message: string } {
  ...
  return { valid: true, message: 'Prototype verification passed. This demo check does not confirm that the UPI ID exists in the banking network.' };
}
