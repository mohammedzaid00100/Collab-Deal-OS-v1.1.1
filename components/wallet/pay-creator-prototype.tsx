...
import { BadgeIndianRupee, CheckCircle2, Clock3, LoaderCircle, LockKeyhole, WalletCards, X } from 'lucide-react';
...
export function PayCreatorPrototype({ creatorName, conversationId, campaignId, creatorProfileId }: {
  creatorName: string;
  ...
  async function confirm() {
    ...
    try {
      const response = await fetch('/api/wallet/prototype-deal-complete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conversationId, campaignId, creatorProfileId, creatorAmount }),
      });
      const payload = await response.json().catch(() => ({})) as { ok?: boolean; message?: string };
      if (!response.ok || !payload.ok) {
        setStatus('idle');
        setError(payload.message ?? 'Could not record the prototype deal completion.');
        return;
      }
      ...
      appendPrototypeTransaction('brand', makePrototypeTransaction({
        ...
        detail: `Creator ₹${creatorAmount.toLocaleString('en-IN')} + 8% platform commission ₹${commission.toLocaleString('en-IN')}`,
        amount: total,
        direction: 'out',
        status: 'Completed',
      }), -total);
      ...
      setStatus('idle');
      setError('Prototype payment service is temporarily unavailable.');
    }
  }
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Pay creator prototype">
      <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl sm:p-7">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.1em] text-violet-700">Prototype payment</p><h2 className="mt-1 text-2xl font-bold tracking-[-0.03em] text-slate-950">Pay {creatorName}</h2><p className="mt-2 text-sm leading-6 text-slate-500">The amount is paid from the brand's prototype wallet. Collab Deal OS adds an 8% platform commission on top.</p></div><button className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-600" type="button" onClick={close} aria-label="Close"><X className="size-4" /></button></div>

        {status === 'success' ? <div className="mt-7 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-center"><CheckCircle2 className="mx-auto size-10 text-emerald-600" /><h3 className="mt-3 text-lg font-bold text-emerald-950">Demo payment confirmed</h3><p className="mt-2 text-sm leading-6 text-emerald-800">₹{creatorAmount.toLocaleString('en-IN')} is the creator amount, ₹{commission.toLocaleString('en-IN')} is the 8% Collab Deal OS commission, and ₹{total.toLocaleString('en-IN')} has been deducted from the brand prototype wallet. This completed deal is now visible in the Developer Tool.</p><button className="mt-5 min-h-11 rounded-xl bg-slate-950 px-5 text-sm font-bold text-white" style={{ color: '#fff' }} type="button" onClick={close}>Done</button></div> : <>
          ...
          <div className="mt-4 flex gap-3 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-800"><Clock3 className="mt-0.5 size-4 shrink-0" /><span>This is the investor prototype. It records the completed deal and wallet movement, but no real bank/UPI payment occurs yet.</span></div>
          ...
          <button className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40" style={{ color: '#fff' }} type="button" disabled={creatorAmount <= 0 || !password || status === 'processing'} onClick={confirm}>{status === 'processing' ? <><LoaderCircle className="size-4 animate-spin" />Recording demo payment…</> : `Confirm demo payment${total ? ` · ₹${total.toLocaleString('en-IN')}` : ''}`}</button>
        </>}
      </div>
    </div> : null}
  </>;
