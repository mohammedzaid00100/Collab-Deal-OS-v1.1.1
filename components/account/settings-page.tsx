...
export async function SettingsPage({ role }: { role: AccountType }) {
  ...
  return <AppShell role={role} displayName={account.displayName ?? role} email={account.email} plan={account.plan}>
    ...
    <p className="mb-7 mt-3 text-sm text-slate-500">Manage your account, workspace appearance, privacy, and wallet access.</p>
    <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
      ...
      <div className="grid gap-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          <h2 className="font-bold text-slate-950">Security and account</h2>
          <p className="mt-3 break-all text-sm text-slate-500">{account.email}</p>
          <nav className="mt-5 grid gap-2 text-sm font-semibold text-violet-700">
            <Link className="py-3" href="/forgot-password">Reset your password</Link>
            <Link className="py-3" href={`/${role}/profile`}>View your profile</Link>
            <Link className="py-3" href={`/${role}/wallet`}>Open wallet</Link>
            <Link className="py-3" href="/privacy">Privacy policy</Link>
          </nav>
        </section>
      </div>
    </div>
