...
export function completePrototypeWithdrawal(role: AccountType, transactionId: string) {
  ...
  const next: PrototypeWalletState = {
    ...
    transactions: current.transactions.map((item) => item.id === transactionId ? { ...item, status: 'Completed' as const, label: 'Withdrawal completed', detail: `${item.detail} · demo payout completed` } : item),
  };
  savePrototypeWallet(role, next);
  return next;
}
